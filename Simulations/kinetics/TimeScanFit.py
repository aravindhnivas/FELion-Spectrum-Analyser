...
    print ("exit program: 0")
    ...
    # Open data and associated parameter file
    if UserChoice == 1:
        DataFilesInDir = glob(directory + "/*_timescan.txt") # picking the timescan files in the directory
        ParameterFilesInDir = glob(directory + "/*_parameter.txt") # picking the parameter file(which contains time steps, temp, etc., )
        for i in range(len(DataFilesInDir)):
                print(str(i) + " : " + sys.path.basename(DataFilesInDir[i]))

        FileNumber = input("Which file would you like to plot? Enter the number of the corresponding file: ")
        datafile = open(DataFilesInDir[int(FileNumber)], 'r')

        try:
            parameterfile = open(ParameterFilesInDir[int(FileNumber)], 'r')
        except IndexError:
            print("Associated parameter file does not exist.")
            break
        except FileNotFoundError:
            print("Associated parameter file does not exist.")

        # Create variables from the variable list and assign values from the parameter file
        ListofParameters = ("Temperature", "Timesteps", "NumberOfMasses", "SimulationTime", "StartTimeStep", "n_He", "k01", "k12", "k23", "k10", "k21", "k32", "k34", "k43")
        parameter_lines = parameterfile.readlines() # just reading all the parameters in the parameterfile
        parameters = [] # parameter read from parameterfile stored here
        parameterlist = {}
        ...
        for i in ListofParameters:
            try:
                locals()[i] = int(parameterlist[i])  # creates variables with the names in the list defined above
            except KeyError:
                print("Warning: " + i + " is not defined in the parameter file")  # Returns a warning if the variable does not exist in the parameter file
            except ValueError:
                try:
                    locals()[i] = float(parameterlist[i])  # If the value of the variable is not an integer, the program tries to read the value as a float
                except ValueError:
