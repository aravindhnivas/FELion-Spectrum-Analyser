...
def depletionPlot(files, location, power_n, dpi, timeIndex, parent):
    ...
    try:
        ...
        for i in range(0, len(counts), 2):
            ...
            axs.errorbar(power_off, depletion_off,
                         yerr=depletion_off_err, fmt='ok')
            axs.errorbar(power_on, depletion_on,
                         yerr=depletion_on_err, fmt='ok')
        ...
        def uy_OFF(x, uN, uK_OFF): return uN*unp.exp(-uK_OFF*x)

        def uy_ON(x, uNa0, uNn0, uK_OFF, uK_ON): return uNa0 * \
        ...
        for i in range(len(N)):
            ...
            depletion, depletion_error = unp.nominal_values(
                udepletion), unp.std_devs(udepletion)
        ...
        def plot(i, l):
            ...
            g_off0, = axs.plot(
                x, y_off0, label='N_OFF: [{:.2f}mJ], K_OFF={:.2fP}/J, N={:.2fP}'.format(power_values[i+1], uK_OFF[i], uN[i]))

            # on plotting
            y_on0 = N_ON((x, K_OFF[i]), Na0[i], Nn0[i], K_ON[i])
            g_on0, = axs.plot(x, y_on0, label='N_ON: [{:.2f}mJ], K_ON={:.2fP}/J, N={:.2fP}, Na0={:.2fP}, Nn0={:.2fP}'.format(
            ...
            koff_g = fig2.add_axes([l, 0.12, 0.2, 0.015], facecolor=axcolor)
            ...
            koff_slider = Slider(
                koff_g, '$K_{OFF}$', 0, K_OFF[i]+10, valinit=K_OFF[i])
            ...
            def update(val):
                ...
                depletion_new1, depletion_error_new1 = unp.nominal_values(
                ...
                legend.get_texts()[k].set_text(
                    'N_OFF: [{:.2f}mJ], K_OFF={:.2fP}/J, N={:.2fP}'.format(power_values[i+1], ukoff, un))
                legend.get_texts()[k+1].set_text('N_ON: [{:.2f}mJ], K_ON={:.2fP}/J, N={:.2fP}, Na0={:.2fP}, Nn0={:.2fP}'.format(
                    power_values[i], ukon, una+unn, una, unn))
                depletion_legend.get_texts()[i].set_text(
                    'A = {:.2fP}, K_ON = {:.2fP}/J'.format(uA_new1, ukon))
        ...
        for i in range(len(N)):
            koff_slider, n_slider, kon_slider, na_slider, nn_slider, koff_g, n_g, kon_g, na_g, nn_g, fig2 = plot(
                i, l)
            l += 0.25
        ...
        depletion_legend = depletion_plot.legend(
            loc='lower right', fontsize=lg_fontsize)
        depletion_plot.set_xlabel('$n * t * E (Joule)$', fontsize=lb_size)
        depletion_plot.set_ylabel(
            'Relative abundance of active isomer', fontsize=lb_size)
        depletion_plot.set_title(
            '$D(ntE) = 1-N_{ON}/N_{OFF}$ fitted with $D(ntE) = A(1-e^{-K_{ON}*ntE})$', fontsize=title_fontsize)
